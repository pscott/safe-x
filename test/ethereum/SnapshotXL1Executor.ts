...
async function baseSetup() {
  ...
  const SnapshotXContract = await ethers.getContractFactory(
    "SnapshotXL1Executor"
  )
  ...
  const initData = masterSnapshotXModule.interface.encodeFunctionData("setUp", [
    encodedInitParams,
  ])
...
async function receiveProposalTest(SnapshotXModule: any) {
  ...
  await SnapshotXModule.receiveProposalTest(
    callerAddress,
    executionHash,
    proposal_outcome,
    [txHash1, txHash2]
...
describe("Snapshot X L1 Proposal Executor:", () => {
  ...
  describe("Setters", async () => {
    ...
    it("The safe can disable Snapshot X module", async () => {
      ...
      expect(await safe.isModuleEnabled(SnapshotXModule.address)).to.equal(
        false
      )
    })
  })
